...
- `/auth/*` still works without the key **from localhost only**, so the browser login flow keeps working. From any other address it needs the key too — `/auth/login` and `/auth/logout` both rewrite the stored tokens. If the proxy is remote, tunnel the port (`ssh -L 42069:localhost:42069 you@host`) and log in through the tunnel. Note that in Docker a browser on the host is *not* loopback as far as the container is concerned. Re-authenticate from inside it instead: `docker compose exec claude-proxy node -e 'fetch("http://127.0.0.1:42069/auth/get-url").then(r=>r.json()).then(console.log)'`, open the printed URL in your browser, then post the `code#state` it gives you back the same way through `/auth/callback?manual_code=...`
